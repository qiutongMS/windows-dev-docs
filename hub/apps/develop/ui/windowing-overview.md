...
> [!NOTE]
> The WinUI [Microsoft.UI.Xaml.Window](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.window) class includes several properties carried over from the UWP `Windows.UI.Xaml.Window` class, but those properties aren't supported in WinUI apps. These properties always have a `null` value and are not used in WinUI apps: `CoreWindow`, `Current`, and `Dispatcher`.
