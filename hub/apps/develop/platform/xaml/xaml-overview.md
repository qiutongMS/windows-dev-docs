...
- [{StaticResource}](staticresource-markup-extension.md): supports referencing resource values that are defined in a [ResourceDictionary](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.resourcedictionary). These resources can be in a different XAML file but must ultimately be findable by the XAML parser at load time. The argument of a `{StaticResource}` usage identifies the key (the name) for a keyed resource in a [ResourceDictionary](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.resourcedictionary).
...
Windows Runtime also supports the [{x:Null} markup extension](x-null-markup-extension.md). You use this to set [**Nullable**](/dotnet/api/system.nullable-1) values to **null** in XAML. For example you might use this in a control template for a [CheckBox](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.controls.checkbox), which interprets **null** as an indeterminate check state (triggering the "Indeterminate" visual state).
...
For example, here's the syntax for referencing a reusable [Style](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.style) from a [ResourceDictionary](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.resourcedictionary): `<Button Style="{StaticResource SearchButtonStyle}"/>`. A **Style** is a reference type, not a simple value, so without the `{StaticResource}` usage, you would've needed a `<Button.Style>` property element and a `<Style>` definition within it to set the [FrameworkElement.Style](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.frameworkelement.style) property.
...
To illustrate, the next XAML example sets the value of the [FrameworkElement.Style](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.frameworkelement.style) property of a [Border](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.controls.border) by using attribute syntax. The `FrameworkElement.Style` property takes an instance of the [Style](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.style) class, a reference type that by default could not be created using an attribute syntax string. But in this case, the attribute references a particular markup extension, [StaticResource](staticresource-markup-extension.md). When that markup extension is processed, it returns a reference to a **Style** element that was defined earlier as a keyed resource in a resource dictionary.
...
When you compile, the compiler now expects that there will be a method named `showUpdatesButton_Click` defined in the code-behind file, in the namespace declared in the XAML page's [x:Class](x-class-attribute.md) value. Also, that method must satisfy the delegate contract for the [Click](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.controls.primitives.buttonbase.click) event. For example:
...
- You might use styles and templates either for templating existing controls, or if you define a control that supplies a default template as part of a control package. When you use it to define styles and templates, the relevant XAML is often declared as a discrete XAML file with a [ResourceDictionary](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.resourcedictionary) root.
...
Here are some tips for defining UI elements in XAML using best practices for performance. Many of these tips relate to using XAML resources, but are listed here in the general XAML overview for convenience. For more info about XAML resources see [ResourceDictionary and XAML resource references](xaml-resource-dictionary.md). For some more tips on performance, including XAML that purposely demonstrates some of the poor performance practices that you should avoid in your XAML, see [Optimize your XAML markup](/windows/uwp/debug-test-perf/optimize-xaml-loading).

- If you use the same color brush often in your XAML, define a [SolidColorBrush](/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.media.solidcolorbrush) as a resource rather than using a named color as an attribute value each time.
...
> [!NOTE]
> Apps that use C++/CX for code don't get the specific **XamlParseException**. But the message in the exception clarifies that the source of the error is XAML-related, and includes context info such as line numbers in a XAML file, just like **XamlParseException** does.
